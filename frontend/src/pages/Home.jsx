...
const Home = () => {
  ...
  const options = [
    ...
    { value: 'react', label: 'React JS' } // ✅ New React Option
  ];
  ...
  const [language, setLanguage] = useState("html"); // ✅ new state for syntax highlighting

  // ✅ Extract code safely + detect language
  function extractCode(response) {
    const match = response.match(/```(\w+)?\n?([\s\S]*?)```/);
    if (!match) return response.trim();

    const lang = match[1] || "html";
    if (lang === "jsx" || lang === "react") setLanguage("javascript");
    else if (lang === "js") setLanguage("javascript");
...
Framework to use: ${frameWork.value === "react" ? "React JS" : frameWork.label}  

Requirements:  
- If framework is React, use functional components (no class) and Tailwind CSS if needed.  
...
- For React, ensure it’s a full working component file (like App.jsx).  
- For HTML, ensure it’s a single .html file.
`
      });

      const codeText = extractCode(response.text);
      setCode(codeText);
  ...
  const downloadFile = () => {
    ...
    const fileName = frameWork.value === "react" ? "GeneratedComponent.jsx" : "GeneratedComponent.html";
        ...
        <div className="w-full py-6 rounded-xl bg-[#141319] mt-5 p-5">
          ...
          <Select
            ...
            onChange={(selected) => {
              ...
              if (selected.value === "react") setLanguage("javascript");
              else if (selected.value.includes("js")) setLanguage("javascript");
            ...
            !outputScreen ? (
              ...
              <>
                ...
                <div className="bg-[#17171C] w-full h-[50px] flex items-center justify-between px-4">
                  <p className='font-bold text-gray-200'>Code Editor ({language.toUpperCase()})</p>
                ...
                <div className="h-full">
                  {tab === 1 ? (
                    <Editor value={code} height="100%" theme='vs-dark' language={language} />
                  ) : (
